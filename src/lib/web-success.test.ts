...
describe("checkWebSuccess", () => {
  ...
  it("returns true only when ALL conditions are satisfied", () => {
    ...
    expect(checkWebSuccess(html, css, js, "hasHTML:card|hasCSS:padding|hasJS:querySelector")).toBe(
      true,
    );
    expect(checkWebSuccess(html, css, js, "hasHTML:card|hasCSS:gap|hasJS:querySelector")).toBe(
      false,
    );
  });
